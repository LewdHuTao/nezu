...
@ApplyOptions<CommandOptions>({
    name: "flip",
    description: "flip text or other user message",
    ...
    async messageRun(message: Message, args: Args) {
        ...
        if (!argument.success && !message.reference) {
            return message.channel.send({
                ...
                embeds: [
                    new MessageEmbed()
                        .setDescription(`⚠ | Please input text or message id to flip`)
        ...
        const resovedFlip = await this.resolveFlip(resolveMessage);
        if (!resovedFlip.output) {
            return message.channel.send({
                reply: {
                    messageReference: message ?? null
                },
        ...
        return message.channel.send({
            embeds: [
                new MessageEmbed()
                    .setTitle("Flip Message")
                    .setDescription(String(resovedFlip.output))
                    .setColor("LUMINOUS_VIVID_PINK")
            ]
        });
    }

    private async resolveFlip(text: string): Promise<jsonWeebyFormatText> {
        const JsonBrowser = await petitio("https://weebyapi.xyz/json/format").query({ text, type: "flip", token: config.weebyToken }).json();
        return JsonBrowser;
    }
}
