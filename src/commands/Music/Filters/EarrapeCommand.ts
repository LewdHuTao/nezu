...
export class clientCommand extends Command {
    async messageRun(message: Message) {
        ...
        audio?.filters.setEarrape(!audio.filters.status.earrape);
        await message.channel.send({
            reply: isEligbleReply(message),
            embeds: [
                new MessageEmbed()
                    .setDescription(`${audioEmoji.CHECK_MARK} | ${audio?.filters.status.earrape ? "Enabled" : "Disabled"} earrape filter`)
