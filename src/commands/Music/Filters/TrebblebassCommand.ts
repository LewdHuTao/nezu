...
export class clientCommand extends Command {
    async messageRun(message: Message) {
        ...
        audio?.filters.setTrebbleBass(!audio.filters.status.trebblebass);
        await message.channel.send({
            reply: isEligbleReply(message),
            embeds: [
                new MessageEmbed()
                    .setDescription(`${audioEmoji.CHECK_MARK} | ${audio?.filters.status.trebblebass ? "Enabled" : "Disabled"} trebblebass filter`)
