...
export class clientContext extends SlashCommand {
    public async messageRun(interaction: BaseGuildCommandInteraction<"present"> & CommandInteraction) {
        ...
        if (audio.playerTimeout) clearTimeout(audio.playerTimeout);
        audio.shoukakuPlayer.connection.disconnect();
        this.container.client.audioManager.queue.delete(interaction.guildId);
        return interaction.editReply({
            embeds: [
                new MessageEmbed()
                    .setDescription(`${audioEmoji.CHECK_MARK} | Disconnected from ${(interaction.member as GuildMember | null)?.voice.channel!.toString()} voice channel`)
                    .setColor("LUMINOUS_VIVID_PINK")
            ]
        });
    }
